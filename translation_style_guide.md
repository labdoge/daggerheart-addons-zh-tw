...
- Add translations only for add-on namespaces such as `DHCS.*` or `DHD.*`; do not duplicate core `DAGGERHEART.*` entries.
...
For hardcoded UI, prefer upstream issues or pull requests. Runtime patches should be small, isolated, version-aware, and documented.
